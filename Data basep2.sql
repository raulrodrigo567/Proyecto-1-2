...
create database pi_raulPenayo;

-- seleccionamos nuestra base de datos
use pi_raulPenayo;

-- creamos las tabla para el dataset homicidios_hechos
...
LOAD DATA LOCAL INFILE 'C:/Users/Raul Penayo/Desktop/p2ori/homicidios_hechos.csv'
INTO TABLE homicide_facts
FIELDS TERMINATED BY ',' ENCLOSED BY '"'
LINES TERMINATED BY '\n' IGNORE 1 LINES;

/*LOAD DATA LOCAL INFILE 'C:/Users/Raul Penayo/Desktop/p2ori/homicidios_víctimas.csv'
...
SET Fecha_fallecimiento = NULLIF(TRIM(@FechaFallecimiento), 'SD'),
    ...
    INSERT INTO homicide_victims (Id_Hecho, Fecha, Año, Mes, Día, Rol, Víctima, Sexo, Edad, Fecha_fallecimiento) 
    VALUES
('2021-0095', '2021-12-30', 2021, 12, 30, 'CONDUCTOR', 'MOTO', 'MASCULINO', 27, '2022-01-02 00:00:00'),
('2021-0096', '2021-12-15', 2021, 12, 15, 'CONDUCTOR', 'AUTO', 'MASCULINO', 60, '2021-12-20 00:00:00');

LOAD DATA LOCAL INFILE 'C:/Users/Raul Penayo/Desktop/p2ori/poblacion_caba.csv'
...
SELECT DISTINCT Id_Hecho
FROM homicide_victims
WHERE Id_Hecho NOT IN (SELECT Id_Hecho FROM homicide_facts);
...
SET Lugar_hecho = UPPER(Lugar_hecho),
    Tipo_calle = UPPER(Tipo_calle),
    Calle = UPPER(Calle),
    Cruce = UPPER(Cruce),
    Dirección_normalizada = UPPER(Dirección_normalizada),
    Participantes = UPPER(Participantes),
    Víctima = UPPER(Víctima),
    Acusado = UPPER(Acusado);
